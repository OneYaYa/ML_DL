...
Original file is located at
    https://colab.research.google.com/drive/1TL0UhWu7CFAn20pDvlIqxq1eR_q8e4v9
...
class BernoulliLSTM(nn.Module):
    def __init__(self, input_dim=28, hidden_dim=128, batch_first=True):
        ...
        # LSTM encoder/sequence model
        self.lstm = nn.LSTM(
            input_size=input_dim,
            hidden_size=hidden_dim,
            batch_first=batch_first
        )
        # Map hidden states at each time step back to pixel probabilities
    ...
    def forward(self, x):
        ...
        ### YOUR SOLUTION GOES HERE ###
        # return ???
        h, _ = self.lstm(x)              # h: [B, T, hidden_dim]
        logits = self.output_layer(h)    # [B, T, D]
        probs = self.sigmoid(logits)     # [B, T, D]
        return probs
...
for epoch in range(epochs):
    ...
    for batch, _ in train_loader:
        ...
        ### YOUR SOLUTION GOES HERE ###
        optimizer.zero_grad()
        output = model(input_seq)              # [B, 27, 28]
        loss = training_loss(output, target_seq)
        loss.backward()
        optimizer.step()
        ...
        for _ in range(n):
            ...
            for _ in range(27):
                ...
                # decoding by deterministic, greedy threshold
                    ...
                    ### YOUR SOLUTION GOES HERE ###
                    sampled_row = (probs >= greedy_threshold).float()
